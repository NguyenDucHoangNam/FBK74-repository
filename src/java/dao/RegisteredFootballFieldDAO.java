...
public class RegisteredFootballFieldDAO extends DBContext {
    ...
    public void insertRFF(RegisteredFootballField rff) {
        ...
        try {
            PreparedStatement st = getConnection().prepareStatement(sql);
            st.setInt(1, rff.getIDRegisteredFootballField());
            st.setInt(2, rff.getAccount1().getIDAccount());
            st.setInt(3, rff.getFootballFieldSchedule().getIDFootballFieldSchedule());
            st.setDate(4, rff.getDate());
            st.setString(5, rff.getName());
            st.setString(6, rff.getPhoneNumber());
            st.setInt(7, rff.getDeposit());
            st.setInt(8, rff.getStatus());
            st.setString(9, rff.getNote());
            st.executeUpdate();
        } catch (SQLException e) {
            System.out.println(e);
        }
    ...
    public static void main(String[] args) {
        ...
        FootballFieldScheduleDAO ffsd = new FootballFieldScheduleDAO();
        //        try {
        //            java.util.Date date = sdf.parse(test);
        //            java.sql.Date sqlDate = new java.sql.Date(date.getTime());
        //            RegisteredFootballField rff = new RegisteredFootballField(4, userd.getUserByIDAccount(2), null, null, ffsd.getFootballFieldScheduleByID(11), sqlDate, "Nguyen van A", "012234567", 100000, 1, "Oknha");
        //            rFFD.insertRFF(rff);
        //        } catch (ParseException ex) {
        //            Logger.getLogger(RegisteredFootballFieldDAO.class.getName()).log(Level.SEVERE, null, ex);
        //        }
